...
namespace Graphic
{
	...
	void Camera::cameraMove(GLFWwindow* m_pWindow)
	{
		...
		if (glfwGetKey(m_pWindow, GLFW_KEY_A) == GLFW_PRESS && !glfwGetKey(m_pWindow, GLFW_KEY_LEFT_CONTROL))	cameraPos -= glm::normalize(glm::cross(cameraFront, cameraUp)) * cameraSpeed;
